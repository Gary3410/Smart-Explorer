...
class COCODetection(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        """
        im, gt, masks, h, w, num_crowds, path = self.pull_item(index)
        #print(gt.shape)
        #print(masks.shape)
        #print(np.unique(masks[0, :, :]))
        #print(num_crowds)
        return im, (gt, masks, num_crowds, path)
    ...
    def pull_item(self, index):
        ...
        return torch.from_numpy(img).permute(2, 0, 1), target, masks, height, width, num_crowds, path
...
def detection_collate(batch):
    ...
    for sample in batch:
        ...
        path.append(sample[1][3])
    return imgs, (targets, masks, num_crowds, path)
